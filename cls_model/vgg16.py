...
OUTPUT_DIR = "tb_results_vgg16"
MODEL_SAVE_PATH = os.path.join(OUTPUT_DIR, "best_vgg16.pth")
EXCEL_SAVE_PATH = os.path.join(OUTPUT_DIR, "vgg16_test_result.xlsx")
CAM_SAVE_DIR = os.path.join(OUTPUT_DIR, "grad_cam_images")

BATCH_SIZE = 16 # VGG16은 메모리를 많이 차지하므로 OOM 발생 시 8로 줄이세요.
LEARNING_RATE = 1e-4
NUM_EPOCHS = 50
PATIENCE = 5
IMG_SIZE = 224
...
def train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs, patience=5):
    print(f"🔥 학습 시작 (Device: {DEVICE}, Model: VGG16)")
...
def evaluate(model, test_loader):
    ...
    # [VGG16 Target Layer] features의 마지막 레이어
...
def main():
    ...
    # VGG16 Classifier 수정 (classifier[6]이 마지막 Linear Layer)
